...
def template4(gen, card_template: str, MprimaryColor: int,
    ...
    # Fill entire card with primary color
    draw.rectangle([0, 0, card_width, card_height], fill=primaryColor)

    # Add subtle pattern to background
    print("Adding background pattern...")
    pattern_spacing = 32
    pattern_size = 8
    pattern_color = gen.lighten_color(primaryColor, 0.1)
    ...
    # Student photo with yellow/gold border
    print("Adding student photo...")
    photo_size = (240, 280)
    photo_position = (60, 250)
    try:
        with Image.open(student_photo) as photo:
            ...
            # Draw the border
            border_position = (photo_position[0] - 1.5, photo_position[1] - 1.5)
            draw.rounded_rectangle([*border_position, border_position[0] + border_size[0],
                                 border_position[1] + border_size[1]], radius=30, fill=secondaryColor)
    ...
    details_rect = [details_x - 30, details_y - 30, card_width - 40, card_height - 150]
    ...
    print("Adding enhanced styling on details I got u...")
    for i, (label, value) in enumerate(fields):
        ...
        draw.text((details_x + 200, details_y), value,
                 fill="white", font=font_medium_bold)

        details_y += spacing

    # Add small watermark logo icons at bottom corners
    print("Adding watermark logo icons...")
    secure_color = (150, 150, 150)
    try:
        watermark_path = files(qure.assets).joinpath("avatar.png")
        with watermark_path.open("rb") as f:
            watermark_logo = Image.open(f).convert("RGBA")

        small_logo = watermark_logo.resize((60, 70), Image.LANCZOS)
        if small_logo.mode != 'RGBA':
            small_logo = small_logo.convert('RGBA')

        # Bottom right corner
        card.paste(small_logo, (card_width - 130, card_height - 130), small_logo)
        ...
        draw.text((card_width - powered_width - 30, card_height - 50),
                powered_text, fill=secure_color, font=font_smallest)
    ...
    # Save both sides
    back_side = template4_back(gen, card_template, MprimaryColor, MsecondarColor,
...
def template4_back(gen, card_template: str, MprimaryColor: int, MsecondarColor: int,
    ...
    try:
        ...
        qr_position = (120, header_height + 90)
    ...
    for line in terms_text:
        draw.text((terms_x, terms_y), line, fill="white", font=font_small)
        terms_y += 25
    ...
    draw.line([(sig_line_start, sig_box_y + 70), (sig_line_end, sig_box_y + 70)], fill="white", width=3)
    ...
    draw.text((name_x, sig_box_y + 80), principal_name, fill="white", font=font_small_bold)
    ...
    # Add small college logo icons at bottom corners
    secure_color = (150, 150, 150)
    try:
        watermark_path = files(qure.assets).joinpath("avatar.png")
        with watermark_path.open("rb") as f:
